...
static int _cli_sned(rec_msg_t *msg, mrpc_connection_t *c)
{
	...
	pbc_slice s = { body_buf, sizeof(*body_buf) };
	...
	header.body_length = body_length + 1; // by protocol we need plus 1
	//TODO: service method
	char header_buf[128];
	pbc_slice s2 = {header_buf, 128};
	...
	int head_len = 128 - r;
	int pkg_len = 12 + body_len + head_len;
...
static void mrpc_ev_after()
{
	...
	list_for_each_entry_safe(c, n, head, list_to) {
		if(now - c->connecting > MRPC_CONNECTION_TO) {
			list_del(&c->list_to);
			_conn_close(c);
			_conn_free(c);
		}
